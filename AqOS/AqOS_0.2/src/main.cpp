...
/*  writeFault|=storage.writeToMemory(writeBuffer, Storage::Slots::APP_EUI);
  ...
  Serial1.println(readBuffer[0]);
  Serial1.println(readBuffer[1]);
  Serial1.println(readBuffer[2]);
  Serial1.println(readBuffer[3]);
  Serial1.println(readBuffer[4]);
  Serial1.println(readBuffer[5]);
  Serial1.println(readBuffer[6]);
...
/*  if (!last_tx || millis() - last_tx > TX_INTERVAL) 
  ...
  */
  ...
  setupWSerial.param(msg);
//  send_packet(msg);
  readFault|=storage.readFromMemory(readBuffer, Storage::Slots::APP_KEY);
